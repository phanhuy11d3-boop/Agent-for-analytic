...
- Anchor the summary directly to the question asked and the PRIMARY evidence kind for that intent:
    ...
    comparison / metric_by_dimension → compare the groups and highlight the biggest gap
    distribution → describe the shape and dominant category
    driver_analysis → describe which factors correlate with the outcome
- Do NOT anchor the summary on a secondary or context-only evidence item.
...
def run_analytics(
    ...
    for ev in (evidence_results or report_spec.get("evidence_results") or []):
        if ev.get("kind") in CHARTABLE and ev.get("success") and ev.get("data"):
            primary_evidence = {"kind": ev["kind"], "title": ev.get("title"), "data_preview": ev["data"][:5]}
            break
...
def analytics_agent_node(state: dict) -> dict:
    ...
    evidence_plan = build_evidence_plan(
        ...
        explicit_aggregate_plan=state.get("explicit_aggregate_plan"),
    )
