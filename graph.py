...
def run(question: str, selected_table: str | None = None) -> AgentState:
    return APP_GRAPH.invoke({"question": question, "selected_table": selected_table, **_INITIAL_STATE})


def stream(question: str, selected_table: str | None = None):
    """Yield (node_name, partial_state) for each agent step."""
    for chunk in APP_GRAPH.stream({"question": question, "selected_table": selected_table, **_INITIAL_STATE}):
