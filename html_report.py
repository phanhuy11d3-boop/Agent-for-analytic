...
def _table_cell_text(key: str, value: Any) -> str:
    if value is None:
        return ""
    if key == "row_count" and isinstance(value, (int, float, decimal.Decimal)):
        return f"{int(value):,}"
    if key == "value" and isinstance(value, (int, float, decimal.Decimal)):
...
def _semantic_proposal_html(proposal: dict[str, Any]) -> str:
    ...
    for label, field in (
        ...
        score_text = f"Confidence: {score}" if score is not None else "Needs confirmation"
        rows.append(
            "<div class=\"mschema-row\">"
            f"<div class=\"name\">{_safe_text(label)}</div>"
            f"<div class=\"meta-line\">{_safe_text(value)}</div>"
            f"<div class=\"meta-line\">{_safe_text(score_text)}</div>"
            "</div>"
        )
    warnings = "".join(f"<li>{_safe_text(w)}</li>" for w in (proposal.get("warnings") or [])[:4])
    if warnings:
        rows.append(f"<ul class=\"warning-list\">{warnings}</ul>")
...
def _claim_contract_html(claim_contract: list[dict[str, Any]]) -> str:
    if not claim_contract:
        return "<div class=\"hint\">No claim contract was generated.</div>"
    rows = []
    for item in claim_contract[:3]:
        evidence = ", ".join(str(eid) for eid in (item.get("evidence_ids") or [])[:5]) or "No executed evidence"
        warning = item.get("warning") or "No additional warning"
        rows.append(
            "<div class=\"mschema-row\">"
            f"<div class=\"name\">{_safe_text(item.get('claim_type') or 'claim')}</div>"
            f"<div class=\"meta-line\">Evidence: {_safe_text(evidence)}</div>"
            f"<div class=\"meta-line\">Semantic source: {_safe_text(item.get('semantic_source'))}</div>"
            f"<div class=\"meta-line\">Status: {_safe_text(item.get('status'))}</div>"
            f"<div class=\"meta-line\">{_safe_text(warning)}</div>"
            "</div>"
        )
    return "".join(rows)
...
def _failure_trace_html(failure_trace: list[dict[str, Any]]) -> str:
    if not failure_trace:
        return "<div class=\"hint\">No pipeline trace was generated.</div>"
    rows = []
    for item in failure_trace:
        rows.append(
            "<div class=\"mschema-row\">"
            f"<div class=\"name\">{_safe_text(item.get('stage'))}: {_safe_text(item.get('status'))}</div>"
            f"<div class=\"meta-line\">{_safe_text(item.get('detail'))}</div>"
            "</div>"
        )
    return "".join(rows)


def _result_table_html(evidence_results: list[dict[str, Any]], limit: int = 30) -> str:
    items = [
        ev for ev in evidence_results
        if ev.get("success", True)
        and ev.get("kind") in {"metric_by_two_dimensions", "metric_by_dimension", "top_n", "distribution"}
        and ev.get("data")
    ][:2]
    if not items:
        return ""
    rows = []
    for item in items:
        metric = item.get("metric") or ("frequency" if item.get("kind") == "distribution" else item.get("title") or "value")
        for row in item.get("data", []):
            rows.append({"metric": metric, **row})
    rows = rows[:limit]
    columns = []
    for key in ("metric", "label", "series", "value", "row_count"):
        if any(key in row for row in rows):
            columns.append(key)
    if not columns:
        return ""
    header = "".join(f"<th>{_safe_text(col.replace('_', ' ').title())}</th>" for col in columns)
    body = "".join(
        "<tr>"
        + "".join(f"<td>{_safe_text(_table_cell_text(col, row.get(col, '')))}</td>" for col in columns)
        + "</tr>"
        for row in rows
    )
    title = "; ".join(str(item.get("title") or item.get("id") or "Aggregate results") for item in items)
    return (
        "<section class=\"insight-card result-card\">"
        f"<div class=\"block-title\">Result Table</div>"
        f"<div class=\"hint\">{_safe_text(title)}. Showing the first {len(rows)} aggregate rows.</div>"
        f"<div class=\"result-table-wrap\"><table class=\"result-table\"><thead><tr>{header}</tr></thead><tbody>{body}</tbody></table></div>"
        "</section>"
    )
...
.result-table-wrap { margin-top:10px; max-height:360px; overflow:auto; border:1px solid var(--line); border-radius:6px; }
.result-table { width:100%; border-collapse:collapse; font-size:13px; background:white; }
.result-table th, .result-table td { padding:9px 10px; border-bottom:1px solid #edf1f5; text-align:left; }
.result-table th { position:sticky; top:0; background:#f8fafc; color:#344054; text-transform:uppercase; font-size:11px; letter-spacing:.04em; }
...
const state = { filters: {}, cross: {}, chartInstances: {} };
...
function updateFilteredSummary(rows) {
  ...
  if (!(SPEC.slicers || []).length) {
    el.textContent = 'Evidence source: executed aggregate/profile results. Embedded rows are only an audit sample.';
    return;
  }
  ...
  el.textContent = `Visible audit sample: ${rows.length.toLocaleString()} of ${total.toLocaleString()} embedded rows. Active filters: ${active}.`;
}
function updateKpis(rows) {
  const root = document.getElementById('kpi-grid');
  if (!root) return;
  const cards = SPEC.summary_cards || [];
  const hasSlicers = (SPEC.slicers || []).length > 0;
  const shouldUseStaticCards = cards.length && (!hasSlicers || SPEC.output_type === 'metric_dimension_report');
  if (shouldUseStaticCards) {
    root.innerHTML = cards.slice(0, 4).map(c => `<div class="card"><div class="kpi-value">${esc(c.value)}</div><div class="kpi-label">${esc(c.label)}</div><div class="kpi-note">${esc(c.note || '')}</div></div>`).join('');
    return;
  }
  ...
  const dynamic = [
    { label: 'Visible Rows', value: rows.length.toLocaleString(), note: 'Rows after interactive filters' },
  ];
...
function renderLineChart(root, chart) {
  if (!window.Chart) {
    root.innerHTML = '<div class="hint">Chart.js not loaded — check network.</div>';
    return;
  }
  const data = chart.data || [];
  if (!data.length) { root.innerHTML = '<div class="hint">No data for trend chart.</div>'; return; }
  const canvasId = 'canvas-' + chart.id;
  root.innerHTML = `<canvas id="${canvasId}" style="width:100%;max-height:280px"></canvas>`;
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  if (state.chartInstances[canvasId]) { state.chartInstances[canvasId].destroy(); }
  state.chartInstances[canvasId] = new window.Chart(canvas, {
    type: 'line',
    data: {
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ticks: { font: { size: 11 } },
          title: { display: !!(chart.y_label), text: chart.y_label || '', font: { size: 11 } }
        }
      }
    }
  });
}
function renderStackedBarChart(root, chart) {
  if (!window.Chart) {
    root.innerHTML = '<div class="hint">Chart.js not loaded - check network.</div>';
    return;
  }
  const rows = chart.data || [];
  if (!rows.length) { root.innerHTML = '<div class="hint">No data for stacked chart.</div>'; return; }
  const labels = [...new Set(rows.map(r => String(r.label ?? 'Unknown')))];
  const series = [...new Set(rows.map(r => String(r.series ?? 'Value')))];
  const canvasId = 'canvas-' + chart.id;
  root.innerHTML = `<canvas id="${canvasId}" style="width:100%;min-height:300px;max-height:360px"></canvas>`;
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  if (state.chartInstances[canvasId]) { state.chartInstances[canvasId].destroy(); }
  const datasets = series.map((name, i) => ({
    ...
    data: labels.map(label => rows
      .filter(r => String(r.label ?? 'Unknown') === label && String(r.series ?? 'Value') === name)
  ...
  state.chartInstances[canvasId] = new window.Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        x: { stacked: true, ticks: { font: { size: 11 } }, title: { display: true, text: label(chart.metric || chart.unit || 'Value'), font: { size: 11 } } },
        y: { stacked: true, ticks: { font: { size: 12 } }, title: { display: !!chart.primary_dimension, text: label(chart.primary_dimension), font: { size: 11 } } }
      }
    }
  });
}
function renderCharts(rows) {
  ...
  root.innerHTML = charts.map((c, i) => `<div class="chart-card"><div class="chart-head"><div class="chart-title">${esc(c.title)}</div><div class="hint">${esc(c.subtitle || c.unit || '')}</div></div><div class="chart-body" id="chart-${i}" style="min-height:${c.type==='line'?'260px':c.type==='stacked_bar'?'330px':'245px'}"></div></div>`).join('');
  charts.forEach((chart, i) => {
    const body = document.getElementById(`chart-${i}`);
    if (chart.type === 'line') renderLineChart(body, chart);
    else if (chart.type === 'stacked_bar') renderStackedBarChart(body, chart);
    else if (chart.type === 'doughnut') renderDonutChart(body, chart, rows);
...
def generate_report(
    ...
    if report_spec.get("mode") in {"context_required", "semantic_confirmation", "cannot_answer"}:
        ...
        proposal = report_spec.get("semantic_proposal", {})
        ...
        mode = report_spec.get("mode")
        title = {
            "semantic_confirmation": "Confirm Semantic Context",
            "cannot_answer": "Cannot Answer Reliably",
        }.get(mode, "Context Required")
        note = {
            "semantic_confirmation": "The system found a possible semantic interpretation, but it needs confirmation before a decision report.",
            "cannot_answer": "The system stopped because the selected table does not support this question as asked.",
        }.get(mode, "The system stopped on purpose because business meaning is not confirmed. This avoids a long but unreliable report.")
        context_rows = [
...
<title>{h(title)} - {h(generated_at)}</title>
<style>{_CSS}</style>
</head>
<body>
  <header class="topbar">
  ...
  <main class="page">
    <section class="answer-card">
      ...
      <div class="hint sample-note">{_safe_text(note)}</div>
    </section>
    ...
    <section class="context-grid">
      ...
      <div class="insight-card">
        <div class="block-title">Suggested Semantic Context</div>
        <div class="hint">Suggestions are generated from schema/profile signals and must be confirmed before outcome claims.</div>
        <div class="mschema-list">{_semantic_proposal_html(proposal)}</div>
      </div>
    </section>
...
"""
    ...
    output_type = report_spec.get("output_type")

    # Evidence-driven: only embed rows if slicers exist
    sample_rows = raw_data[:MAX_EMBED_ROWS] if slicers else raw_data[:5]
    evidence_sql_html = _evidence_sql_html(evidence_results, sql_query)
    has_distribution_result = any(
        ev.get("success", True) and ev.get("kind") == "distribution" and ev.get("data")
    ...
    if metric_cards:
        cards_html = "".join(
            f'<div class="card"><div class="kpi-value">{_safe_text(c["value"])}</div>'
            f'<div class="kpi-label">{_safe_text(c["label"])}</div></div>'
            for c in metric_cards[:4]
        )
        metrics_block = f'<section class="kpi-grid" id="kpi-grid">{cards_html}</section>'
    ...
    if executive_points:
        ...
        if candidate_signals:
            signals_html = "".join(
                "<div class=\"signal\">"
                f"<div class=\"signal-head\"><span class=\"signal-name\" title=\"{_safe_text(s.get('column'))}\">"
                f"{_safe_text(s.get('rank'))}. {_safe_text(s.get('column'))}</span>"
                f"<span class=\"signal-score\">Score {_safe_text(s.get('score'))}</span></div>"
                f"<div class=\"signal-meta\">{_safe_text(s.get('role'))} - Missing {_safe_text(s.get('missing'))}</div>"
                f"<div class=\"hint\">{_safe_text(s.get('reason'))}</div>"
                "</div>"
                for s in candidate_signals[:5]
            )
            insights_block = f"""
    ...
    # --- BLOCK: Charts + Slicers (only if charts or slicers exist) ---
    viz_block = ""
    if charts:
        if slicers:
            viz_block = """
    <div class="dashboard">
      <aside class="slicer-pane">
        ...
        <div class="hint">Filters affect the embedded sample and supporting charts.</div>
        <div id="slicers"></div>
      </aside>
      <section><div class="chart-grid" id="chart-grid"></div></section>
    </div>"""
        else:
            viz_block = """
    <section><div class="chart-grid" id="chart-grid"></div></section>"""
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Maxxem Report - {h(generated_at)}</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.4/dist/chart.umd.min.js"></script>
<style>{_CSS}</style>
</head>
<body>
  <header class="topbar">
    <h1>Maxxem Data Analysis</h1>
    <div class="question">{_safe_text(question)}</div>
    <div class="meta">Generated {h(generated_at)}</div>
  </header>
  ...
  <main class="page">
    ...
    <section class="answer-card">
      <div class="answer-title">AI Answer</div>
      <div class="answer-text">{_safe_text(direct_answer)}</div>
      <div class="hint sample-note" id="filtered-summary"></div>
    </section>
    ...
    <section class="audit-grid">
      ...
      <div class="audit-card">
        <div class="block-title">Scope &amp; Limitations</div>
        <div class="hint">Total rows: {_safe_text(scope.get("total_rows"))}; profiled columns: {_safe_text(scope.get("profiled_columns"))}.</div>
        <ul class="warning-list">{warning_html or '<li>No major limitations detected.</li>'}</ul>
        <div class="block-title" style="margin-top:14px">Pipeline Trace</div>
        {_failure_trace_html(report_spec.get("failure_trace", []))}
        <div class="block-title" style="margin-top:14px">Claim Contract</div>
        {_claim_contract_html(report_spec.get("claim_contract", []))}
      </div>
    </section>
